                ...
                for hd in [config.hd]:
                    ...
                    for t in range(config.repeat_s,config.repeat_e):
                        ...
                        for fi in range(config.fold_s,config.fold_e):
                            ...
                            model=SAG(hidden_dim=hd,SAG_ratio=sag_r,CONV_TYPE=conv_type,act_op=config.act_op,before_pooling_layer=config.before_layer,after_pooling_layer=config.after_layer,num_K=ki).to(device)
                            if t==0 and fi==0:
